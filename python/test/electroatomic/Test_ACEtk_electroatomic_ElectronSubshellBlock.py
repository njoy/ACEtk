...
# local imports
from ACEtk.electroatomic import ElectronSubshellBlock

class Test_ACEtk_electroatomic_ElectronSubshellBlock( unittest.TestCase ) :
    ...
    def test_component( self ) :
        ...
        def verify_chunk( self, chunk ) :
            ...
            self.assertEqual( 1, chunk.ID[0] )
            self.assertEqual( 2, chunk.ID[1] )
            self.assertEqual( 3, chunk.ID[2] )
            self.assertEqual( 4, chunk.ID[3] )
            self.assertEqual( 5, chunk.ID[4] )
