...
SCENARIO("test interpretation::DEDX1"){
  ...
  GIVEN("A faux stopping power data table"){
    ...
    WHEN("querying for standard stopping power model without a cutoff"){
      ...
      // 	auto makeRef = [](auto x){ return ranges::views::repeat_n(x, 2); };
      // 	GIVEN("each boundary density in range"){
      // 	  auto reference =
      // 	    ranges::views::iota(0,3)
      // 	    | ranges::views::transform( makeRef );
      ...
      // 	  auto trial =
      // 	    standard.densities() |
      // 	    ranges::views::slice(0, 3) |
      // 	    ranges::views::transform( firstTemp );

      // 	  auto results = ranges::views::zip_with( ranges::equal, reference, trial );
      ...
      // 	  auto trial =
      // 	    standard.densities() |
      // 	    ranges::views::slice(0, 2) |
      // 	    ranges::views::transform( firstDensity ) ;
      ...
      // 	auto makeRef = [](auto x){ return ranges::views::repeat_n(x * 3, 2); };
      // 	GIVEN("each boundary temperature in range"){
      // 	  auto reference =
      // 	    ranges::views::iota(0,4)
      // 	    | ranges::views::transform( makeRef );
