...
SCENARIO("test AtomicLevels"){  
  ...
  GIVEN("An ACE Table for 1000.e03"){
    ...
    auto atomicLevel =
      interpretation::EL03::AtomicLevels( table );

    REQUIRE( atomicLevel.values()[0].first == 1 );    
    REQUIRE( atomicLevel.values()[0].second == 13.6 * electronVolt );
    REQUIRE( atomicLevel.isConductive() == false );
  ...
  GIVEN("An ACE Table for 79000.e03"){
    ...
    for ( const auto pair : ranges::view::zip( atomicLevels.values(), compareThese ) ) {
      ...
      auto bindErgRef = std::get<1>(pair.second);
      REQUIRE( std::get<1>(pair.first).value == Approx( bindErgRef ).epsilon(1e-15) ); 

    }

    REQUIRE( atomicLevels.isConductive() == true );    
